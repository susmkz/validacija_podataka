        ...
        if izabraniKanal != None:
            ...
            if filename:
                ...
                try:
                    ...
                    # zapis u file
                    tablica.to_csv(filename, sep=';', decimal=",") #forsiraj decimalni separatoru ","
                except Exception as err:
                    logging.error(str(err), exc_info=True)
                finally:
                    # turn off wait cursor
        ...
        if izabraniKanal != None:
            ...
            if filename:
                ...
                try:
                    ...
                    # slomi se na nazivima fileova koji imaju utf-8 znakove
                    #tablica = pd.read_csv(filename, sep=';', parse_dates=['vrijeme'])
                    #TODO! dirty fix - prosljedi open file handle i forsiraj zarez kao decimalni separator... 
                    tmpFileHandle = open(filename)
                    tablica = pd.read_csv(tmpFileHandle, sep=";", decimal=",", parse_dates=['vrijeme'])
                    ...
                    jedinica = jedinica.strip() #izbjegavanje razmaka...
                    aktivna_jedinica = self.centralniWidget.AKTIVNE_JEDINICE
                    # fix za ucitavanje korekcija iz ppm sfere...
                    if jedinica == 'ppm':
                        jedinica = 'ppb'
                    if jedinica == 'mg/m3':
                        jedinica = 'ug/m3'
                    ...
                    # turn off wait cursor
                    #TODO! nastavak fixa... moram zatvoriti file handle
                    tmpFileHandle.close()
                    QtWidgets.QApplication.restoreOverrideCursor()
